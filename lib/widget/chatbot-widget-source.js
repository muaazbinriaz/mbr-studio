 ...
 *
 * Served dynamically by app/chatbot.js/route.ts (not a static /public
 * file) so the response can be cached/versioned independently of a
 * build. This file is genuinely plain JavaScript — no bundler, no
 * React/Preact — because the whole UI is simple enough that hand-
 * rolled DOM manipulation stays well under the 60kb gzipped budget
 * (this file is a few KB raw, gzip makes it smaller still).
 *
 * ISOLATION STRATEGY: a custom element (<mbr-chat-widget>) with a
 * Shadow DOM root. This avoids CSS collisions with the host page and
 * loads faster than an iframe's second document context, with no
 * postMessage plumbing needed for resizing. An iframe gives stronger
 * isolation from a *hostile* host page, but that's not the threat
 * model here — small businesses embed this on their own sites.
 */
(function () {
  "use strict";
  ...
  if (!CLIENT_KEY) {
    console.error("[MBR Chat Widget] Missing data-client attribute — widget not mounted.");
    return;
  }
  ...
  function getVisitorId() {
    try {
      ...
      // localStorage unavailable (privacy mode, etc.) — per-load fallback.
  ...
  async function fetchConfig() {
    ...
    var res = await fetch(API_BASE + "/api/widget/config?client=" + encodeURIComponent(CLIENT_KEY));
  ...
  // Reimplementation of components/chatbot/useFocusTrap.ts's behavior in
  // vanilla JS, since that hook can't be imported into this bundle.
  ...
  MBRChatWidget.prototype.connectedCallback = async function () {
    ...
    try {
      ...
      // Fail silently — log only, never break the host page.
  ...
  MBRChatWidget.prototype._render = function () {
    ...
    launcher.setAttribute("aria-label", "Open chat with " + (cfg.orgName || "us"));
    launcher.innerHTML =
    ...
    // "Remove branding" is intentionally NOT configurable at any current
    // plan tier — see Prompt 02 Section 5.4. This is a monetizable
    // upsell reserved for Prompt 08 (billing/plans).
    footerLink.href = API_BASE + "/?utm_source=widget&utm_medium=powered_by&utm_campaign=" + campaign;
    ...
    this._els = { root: root, launcher: launcher, panel: panel, closeBtn: closeBtn, messages: messages, input: input, sendBtn: sendBtn, inputRow: inputRow };
  ...
  MBRChatWidget.prototype._sendMessage = async function (text) {
    ...
    try {
      ...
      if (!res.ok) {
        var errBody = contentType.indexOf("application/json") !== -1 ? await res.json().catch(function () { return {}; }) : {};
        self._addBubble("assistant", errBody.error || "Something went wrong — please try again.");
        self._setBusy(false);
        return;
      }

      if (contentType.indexOf("application/json") !== -1) {
        // Non-streaming JSON path — currently only the monthly-limit-reached case.
        var data = await res.json();
        self._addBubble("assistant", data.reply || "Sorry, I couldn't process that.");
        self._setBusy(false);
        return;
      }

      // Streaming plain-text path — the normal case.
      if (res.body && res.body.getReader) {
        ...
        if (!full.trim()) bubble.textContent = "Sorry, I didn't catch that — could you rephrase?";
      } else {
      ...
      self._addBubble("assistant", "Connection issue — please try again in a moment.");
    }
